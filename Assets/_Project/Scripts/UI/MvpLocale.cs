...
public static class MvpLocale
{
    ...
    static readonly Dictionary<string, string[]> Strings = new()
    {
        ...
        ["all_aboard"] = new[] { "All aboard {0}/{1}", "全员就绪 {0}/{1}" },
        ["waiting_team"] = new[] { "Waiting... {0}/{1}", "等待中... {0}/{1}" },
        ["driver_waiting"] = new[] { "Driver waiting up front", "驾驶员在前方等候" },
        ["press_e_drive"] = new[] { "Press [E] to drive", "按 [E] 驾驶" },
        ["press_space_depart"] = new[] { "[Space] Depart", "[空格] 出发" },
        ["press_x_leave"] = new[] { "[X] Leave seat", "[X] 离开座位" },
        ["departing_in"] = new[] { "Departing in {0}… (stragglers left behind)", "即将出发 {0}…（掉队者将被留下）" },
        ["wait_all_aboard"] = new[] { "Everyone must be aboard to depart", "所有人上车后方可出发" },
        ["sit_in_van"] = new[] { "Press [E] to board and sit", "按 [E] 上车就坐" },
        ["dispatch_outbound"] = new[] { "Dispatching to site", "正在前往任务地点" },
        ["return_office"] = new[] { "Returning to office", "正在返回事务所" },
