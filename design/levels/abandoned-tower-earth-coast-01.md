# Level: Earth Coast No.1 — Abandoned Pre-Sale Tower
...
- **Prerequisite**: Office computer accepts the "Retrieve Property Scale Model" commission; team boards dispatch van
...
- **Building identity**: A pre-exodus luxury pre-sale tower, **"Ares Pre-Sale · Earth Coast No.1"**,
  marketed as "owning the last *real* Earth coastline home." Buyers emigrated to Mars;
  the project went **abandoned mid-construction in 2071**. Now inside a Noble Plague
...
- **The commission (satire core)**: A Mars client (a "Earth's Unbuilt Dream" themed
  exhibition / private collector) pays well to retrieve the **sales scale model**
  — a miniature of the Earth future that was never built. They want the *unfinished*
  dream as a collectible: the abandoned state itself is the luxury.
- **Emotional target**: curiosity → unease. "This place was sold to people who left,
  and now their descendants want the leftovers."
- **Lore discoveries**: A temporary isolation notice dated *before* the official infection
  announcement (optional evidence → ties to the Noble Plague truth thread, like the school ledger).

### Settlement Text (satire payoff — draft)

```text
Scale Model Completeness: 91%        Project Status: Halted 2071
Van Dustproofing: Passed             Structural Dust Cleaning Fee: Deducted
Rail Quarantine Fee: Deducted        Personnel Fall/Infection Allowance: None
Client Use: Mars "Earth's Unbuilt Dream" themed exhibition · Private collection
Client Review: The unfinished state is the most authentic — worth more as a collectible than if it had been completed.
```
...
FLOOR 2 (Show-flat / Sales floor — HIGH RISK)
  [F2_L5_DeepTargetArea] (R!B)  <-- scale model objective + monster nest
        |  (LeftLowerConnector)
...
1. **[S] F1_S1_StartVanArea** — van drops team outside the construction site hoarding. Banners:
   "Own a Real Earth Coastline Home" / "Earth's Last Scarce Residences". Route signage points to lobby.
2. **F1_M1_LobbySecurityPassage** — dead reception, sales hall. Players learn the
   ...
   (Diegetic readout on the security desk: "Floor 2 Sales Area: Power Off / Roller Shutter Locked".)
...
6. **Grab the scale model (objective, step 2 / pressure trigger)** — heavy two-hand carry.
...
8. **[E] Return to F1_S1 van** — full return with scale model = full settlement; early
...
|------|-------------------|--------|---------------|
| F1_M3_MainWorkerDorm | none | **Evidence**: Temporary Isolation Notice (photo → bonus side payment) | NorthCorridor signage |
...
|----------|------|-------------|---------|
| F1_M1_LobbySecurityPassage | Story/gate signpost | Sales hall, "Power Off" readout | Teaches the power gate + tone |
...
| F2_L5_DeepTargetArea | Objective + nest | Scale model on a pedestal; monster nest | Climax / pressure trigger |
...
| E-02 | F2_M5_ScaffoldBridge | Environmental: fall/gap (no rail) | — | Carrier with scale model is slow → tense crossing |

**Monster — Infected Site Inspector**: hard hat + hi-vis vest +
...
| EV-01 | F1_M3_MainWorkerDorm | Evidence | Pre-dated Isolation Notice | Photo interaction (bonus) |
...
- **Climax**: grab scale model → monster active → heavy carry descent. Peak tension at scaffold/stairs.
...
|----------|-----------|-------------|
| F1_S1_StartVanArea (van) | Route arrow + job number | "COMMISSION BC-12 / Outsourced Construction Site → Target: Scale Model" stenciled on the fence |
| F1_M1_LobbySecurityPassage | BC-12 form pinned to reception | Commission intake form with job reference and "Client: Mars Private Collector" |
| F2 security shutter (post-power) | "DEBT ACCESS ONLY" | Signage applied to the shutter frame — the commission office's access authorization |
| F1_S3_PowerRoom entrance | BC-12 equipment tag | Tagged breaker panel: "Dangerous Operation / Outsourced Accident Team BC-12 / Circuit Breaker" |
| F2_L5_DeepTargetArea | Overdue payment notice | Stamp-red "INVOICE OVERDUE" / "Outstanding Balance" notice posted near the objective pedestal |
...
|------|----------|-----------|------|-------------|
| Sales Scale Model (objective) | F2_L5_DeepTargetArea | High (lit pedestal) | Lit show-flat beacon | Full settlement |
| Temporary Isolation Notice (evidence) | F1_M3_MainWorkerDorm | Medium | Pinned to dorm board | Bonus payout |
...
| Scale model carry move-speed multiplier | 0.55× | Heavy; carrier vulnerable, others must protect |
| Carrier hotbar lock while carrying | true | Can't use items while two-hand carrying |
| Scale model can be dropped/handed off | true | Co-op relay; drop near van = partial; re-grab allowed |
...
| Evidence photo bonus | +(small) side payment | Mirror school ledger value |
| Partial settlement (evidence only, no scale model) | partial % | Reuse early-return partial path |

## Edge Cases

- **Carrier downed mid-carry**: scale model drops in place; any teammate can pick it up.
- **Scale model dropped off the scaffold/shaft**: define a recovery rule (respawn at last
...
- **Early return with scale model not yet grabbed**: partial settlement (existing path).
...
3. Scale model in F2_L5 can be grabbed only by an alive player at range; pickup syncs to all
   and aggros the monster.
4. Carrying scale model slows the carrier and locks their hotbar; it can be dropped/handed off;
   carrier identity is server-validated (a client cannot remotely "finish").
5. Full return with scale model to the van = full settlement; early return without it = partial;
...
- **Next steps**: `/design-review` this doc → (optional) `/asset-spec` for scale model + inspector
